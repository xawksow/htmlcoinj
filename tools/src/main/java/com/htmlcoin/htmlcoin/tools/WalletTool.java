...
/**
 * A command line tool for manipulating wallets and working with HTMLcoin.
 */
public class WalletTool {
    ...
    public static class Condition {
        ...
        public boolean matchHTMLcoins(BigInteger comparison) {
            try {
        ...
        if (location.startsWith("http") || location.startsWith("htmlcoin")) {
            try {
                ...
                if (location.startsWith("http")) {
                    ...
                    HTMLcoinURI paymentRequestURI = new HTMLcoinURI(location);
                    future = PaymentSession.createFromHTMLcoinUri(paymentRequestURI, verifyPki);
                }
    ...
    private static void wait(WaitForEnum waitFor) throws BlockStoreException {
        ...
        switch (waitFor) {
            ...
            case BALANCE:
                ...
                if (condition.matchHTMLcoins(wallet.getBalance(Wallet.BalanceType.ESTIMATED))) {
                    latch.countDown();
                    break;
                }
                wallet.addEventListener(new AbstractWalletEventListener() {
    ...
    private static void deleteKey() {
        ...
        if (pubkey != null) {
            ...
            try {
                ...
                System.err.println(addr + " does not parse as a HTMLcoin address of the right network parameters.");
                return;
            }
        }
        if (key == null) {

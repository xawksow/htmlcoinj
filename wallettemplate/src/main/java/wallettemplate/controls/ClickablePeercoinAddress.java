...
/**
 * A custom control that implements a clickable, copyable HTMLcoin address. Clicking it opens a local wallet app. The
 * address looks like a blue hyperlink. Next to it there are two icons, one that copies to the clipboard and another
 * that shows a QRcode.
 */
public class ClickableHTMLcoinAddress extends AnchorPane {
    ...
    public String uri() {
        return HTMLcoinURI.convertToHTMLcoinURI(getAddress(), null, Main.APP_NAME, null);
    }
    ...
    protected void showQRCode(MouseEvent event) {
        ...
        final Main.OverlayUI<ClickableHTMLcoinAddress> overlay = Main.instance.overlayUI(pane, this);

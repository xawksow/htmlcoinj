...
/**
 * <p>Provides a standard implementation of a HTMLcoin URI with support for the following:</p>
 *
 * <ul>
...
public class HTMLcoinURI {
     ...
     */
    private static final Logger log = LoggerFactory.getLogger(HTMLcoinURI.class);
    ...
    public static final String HTMLcoin_SCHEME = "htmlcoin";
    ...
    /**
     * Constructs a new HTMLcoinURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws HTMLcoinURIParseException if the URI is not syntactically or semantically valid.
     */
    public HTMLcoinURI(String uri) throws HTMLcoinURIParseException {
        this(null, uri);
    }

    /**
     * Constructs a new object by trying to parse the input as a valid HTMLcoin URI.
     *
     ...
     *
     * @throws HTMLcoinURIParseException If the input fails HTMLcoin URI syntax and semantic checks.
     */
    public HTMLcoinURI(@Nullable NetworkParameters params, String input) throws HTMLcoinURIParseException {
        checkNotNull(input);
        ...
        if (input.startsWith("htmlcoin://")) {
            schemeSpecificPart = input.substring("htmlcoin://".length());
        } else if (input.startsWith("htmlcoin:")) {
            schemeSpecificPart = input.substring("htmlcoin:".length());
        } else {
            throw new HTMLcoinURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new HTMLcoinURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
        ...
        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a HTMLcoin address for this network
            try {
                Address address = new Address(params, addressToken);
                putWithValidation(FIELD_ADDRESS, address);
            } catch (final AddressFormatException e) {
                throw new HTMLcoinURIParseException("Bad address", e);
            }
        }

        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new HTMLcoinURIParseException("No address and no r= parameter found");
        }
    }
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws HTMLcoinURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new HTMLcoinURIParseException("Malformed HTMLcoin URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new HTMLcoinURIParseException("Malformed HTMLcoin URI - empty name '" +
                        nameValuePairToken + "'");
     ...
     */
    private void putWithValidation(String key, Object value) throws HTMLcoinURIParseException {
        if (parameterMap.containsKey(key)) {
            throw new HTMLcoinURIParseException(String.format("'%s' is duplicated, URI is invalid", key));
        } else {
    ...
    /**
     * The HTMLcoin Address from the URI, if one was present. It's possible to have HTMLcoin URI's with no address if a
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("HTMLcoinURI[");
        boolean first = true;
    ...
    public static String convertToHTMLcoinURI(Address address, BigInteger amount, String label, String message) {
        return convertToHTMLcoinURI(address.toString(), amount, label, message);
    }

    /**
     * Simple HTMLcoin URI builder using known good fields.
     * 
     * @param address The HTMLcoin address
     * @param amount The amount in nanocoins (decimal)
     * @param label A label
     * @param message A message
     * @return A String containing the HTMLcoin URI
     */
    public static String convertToHTMLcoinURI(String address, @Nullable BigInteger amount, @Nullable String label,
        ...
        builder.append(HTMLcoin_SCHEME).append(":").append(address);

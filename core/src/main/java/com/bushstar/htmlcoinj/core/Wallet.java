...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider {
        ...
        for (TransactionInput input : tx.getInputs()) {
            ...
            if (result == TransactionInput.ConnectionResult.ALREADY_SPENT) {
                if (fromChain) {
                    ...
                    log.warn("{}: {}", tx.getHash(), new String(Hex.encode(tx.unsafeHTMLcoinSerialize())));
                    Transaction other = input.getConnectedOutput().getSpentBy().getParentTransaction();
                    log.warn("{}: {}", other.getHash(), new String(Hex.encode(tx.unsafeHTMLcoinSerialize())));
                }
    ...
    public static class SendResult {
        /** The HTMLcoin transaction message that moves the money. */
    ...
    public static class SendRequest {
         ...
         * is how mining is incentivized in later years of the HTMLcoin system when inflation drops. It also provides
         ...
         * 
         * <p>HTMLcoin: This is zero even though we add a required fee. This is an extra fee on top that we don't want.</p>
         */
     ...
     *
     * @param address       The HTMLcoin address to send the money to.
    ...
    private class FeeCalculation {
        ...
        public FeeCalculation(SendRequest req, BigInteger value, List<TransactionInput> originalInputs,
            ...
            while (true) {
                ...
                // If change is < 0.01 HTML5, we will need to discard the change
     ...
     * sending transactions to the HTMLcoin network by default. Some sendCoins methods let you specify a broadcaster

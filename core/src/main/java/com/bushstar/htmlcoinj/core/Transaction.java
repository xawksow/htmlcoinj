 ...
 * the minting of new coins. A Transaction object corresponds to the equivalent in the HTMLcoin C++ implementation.</p>
 *
 * <p>Transactions are the fundamental atoms of HTMLcoin and have many powerful features. Read
 ...
 *
 * <p>All HTMLcoin transactions are at risk of being reversed, though the risk is much less than with traditional payment
...
public class Transaction extends ChildMessage implements Serializable {
    ...
    /**
     * Used by HTMLcoinSerializer.  The serializer has to calculate a hash for checksumming so to
     ...
     * value is determined by a formula that all implementations of HTMLcoin share. In 2011 the value of a coinbase
     ...
     * and requires external synchronization. Please refer to general documentation on HTMLcoin scripting and contracts
     ...
     * features of HTMLcoin are developed.</p>
     *
    ...
    public synchronized Sha256Hash hashForSignature(int inputIndex, byte[] connectedScript, byte sigHashType) {
        ...
        try {
            ...
            // is a legacy holdover from a previous, broken design of executing scripts that shipped in HTMLcoin 0.1.
            ...
            if ((sigHashType & 0x1f) == (SigHash.NONE.ordinal() + 1)) {
                ...
                if (inputIndex >= this.outputs.size()) {
                    ...
                    // HTMLcoin implementation. The reference client also contains a bug in handling this case:
     ...
     * since HTMLcoin 0.8+ a transaction that did not end its lock period (non final) is considered to be non

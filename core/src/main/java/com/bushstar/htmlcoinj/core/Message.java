...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the HTMLcoin proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param msg HTMLcoin protocol formatted byte array containing message content.
     * @param offset The location of the first msg byte within the array.
     * @param protocolVersion HTMLcoin protocol version.
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = htmlcoinSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom HTMLcoin protocol.
    ...
    /**
     * Should only used by HTMLcoinSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by HTMLcoinSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeHTMLcoinSerialize()}, which is safe to mutate.
    ...
    public byte[] htmlcoinSerialize() {
        byte[] bytes = unsafeHTMLcoinSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the htmlcoin wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeHTMLcoinSerialize() {
        ...
        try {
            htmlcoinSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to htmlcoinSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the htmlcoin wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void htmlcoinSerialize(OutputStream stream) throws IOException {
    ...
    void htmlcoinSerializeToStream(OutputStream stream) throws IOException {
        log.error("Error: {} class has not implemented htmlcoinSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so HTMLcoinSerializer can avoid 2 instanceof checks + a casting.
     */

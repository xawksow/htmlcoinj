...
public class Utils {
    ...
    /** The string that prefixes all text messages signed using HTMLcoin keys. */
    public static final String HTMLcoin_SIGNED_MESSAGE_HEADER = "HTMLcoin Signed Message:\n";
    public static final byte[] HTMLcoin_SIGNED_MESSAGE_HEADER_BYTES = HTMLcoin_SIGNED_MESSAGE_HEADER.getBytes(Charsets.UTF_8);

    // TODO: Replace this nanocoins business with something better.

    /**
     * How many "nanocoins" there are in a HTMLcoin.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using HTMLcoin.
    ...
    /**
     * How many "nanocoins" there are in 0.01 HTMLcoins.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using HTMLcoin.
     * The term nanocoin is very misleading, though, because there are only 100 million
     * of them in a coin (whereas one would expect 1 billion).
    ...
    public static BigInteger toNanoCoins(String coins) {
        ...
        if (bigint.compareTo(NetworkParameters.MAX_MONEY) > 0)
            throw new ArithmeticException("Amount larger than the total quantity of HTMLcoins possible specified.");
        return bigint;
    }
     ...
     * standard procedure in HTMLcoin. The resulting hash is in big endian form.
     */
    ...
    public static String htmlcoinValueToPlainString(BigInteger value) {
        ...
        BigDecimal valueInHTML5 = new BigDecimal(value).divide(new BigDecimal(Utils.COIN));
        return valueInHTML5.toPlainString();
    }
     ...
     *
     * <p>HTMLcoin only uses this "compact" format for encoding difficulty targets, which are unsigned 256bit quantities.
     ...
     *
     * <tt><p>[24] "HTMLcoin Signed Message:\n" [message.length as a varint] message</p></tt>
     */
    public static byte[] formatMessageForSigning(String message) {
        try {
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            bos.write(HTMLcoin_SIGNED_MESSAGE_HEADER_BYTES.length);
            bos.write(HTMLcoin_SIGNED_MESSAGE_HEADER_BYTES);

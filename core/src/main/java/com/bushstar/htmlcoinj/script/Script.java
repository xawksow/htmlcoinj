...
import static com.bushstar.htmlcoinj.core.Utils.bytesToHexString;
...
public class Script {
     ...
     * The concept of a "from address" isn't well defined in HTMLcoin and you should not assume the sender of a
     ...
     * logic is defined by <a href="https://github.com.bushstar/bips/blob/master/bip-0016.mediawiki">BIP 16</a>.</p>
     *
    ...
    private static void executeCheckSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        try {
            TransactionSignature sig  = TransactionSignature.decodeFromHTMLcoin(sigBytes, false);
    ...
    private static int executeMultiSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        while (sigs.size() > 0) {
            ...
            try {
                TransactionSignature sig = TransactionSignature.decodeFromHTMLcoin(sigs.getFirst(), false);
                Sha256Hash hash = txContainingThis.hashForSignature(index, connectedScript, (byte) sig.sighashFlags);
